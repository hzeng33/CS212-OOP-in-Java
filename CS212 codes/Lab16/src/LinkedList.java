...
public class LinkedList {
	...
	public void prepend(String d) {
		...
		ListNode newNode = new ListNode(d);
		ListNode p = first.next;  //temporary node p becomes the second node.
	...
	public String toString() {
		ListNode p = first.next; //since the first node always be a dummy node, so points to the second node.
		String returnString = "";
		while (p != null) {
			returnString += p.data + " ";
			p = p.next;
